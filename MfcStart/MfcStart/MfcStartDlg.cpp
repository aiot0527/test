...
#include <iostream>	// 이렇게 위에 써줘
using namespace std;

#ifdef _DEBUG
#define new DEBUG_NEW
#pragma comment(linker, "/entry:WinMainCRTStartup /subsystem:console")	//콘솔창 띄우기(디버그 모드에서 항상 콘솔이 나오도록)
#endif
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_EDIT_NUM, m_nNum1);
...
BOOL CMfcStartDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_STATIC_RESULT, _T("0"));	//계산식 결과 초기값 0으로 설정
...
void CMfcStartDlg::OnBnClickedBtnTest()	//버튼 클릭시
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	//AfxMessageBox(_T("hello windows")); //메시지 창을 띄움
	//UpdateData(TRUE);
	//int nSum = 0;
	//for (int i = 0; i < m_nNum1; i++) {	//0부터 m_nNum까지 수의 합
	//	cout << i << endl;	//디버깅하는 방법 알아두기
	//	nSum += i;
	//}
	//m_nNum1 = nSum;
	//
	...
	UpdateData(TRUE);
	int nResult = 0;
	nResult = m_nNum1 + m_nNum2;
	SetDlgItemInt(IDC_STATIC_RESULT, nResult);	//더하기 계산 결과
	UpdateData(false);
